...
class TestDocumentWeb(WebTest):
	...
	def test_url_shows_document(self):
		...
		document = baker.make(InformationDocument, title_en=title)
...
class TestVersions(WebTest):
	...
	def test_save_version(self):
		...
		form.set('title_en', 'Bibi Blocksberg')
		form.set('text_en', new_string)
...
class TestPermissions(WebTest):
	...
	def setUpTestData(cls):
		...
		baker.make(InformationDocument)
...
class TestUnlinkedList(WebTest):
	...
	def setUpTestData(cls):
		cls.user = baker.make(UserProfile, is_superuser=True)
		cls.informationdocument1 = baker.make(InformationDocument, title_de='title_de', title_en='title_en')
		cls.informationdocument2 = baker.make(InformationDocument, text_en="Lorem ipsum [link](document:{}).".format(cls.informationdocument1.id))
		cls.menu_item = baker.make(MenuItem, document=cls.informationdocument2)
